...
#include "CrowdBakerInstancePlaybackHelpers.generated.h"

class UCrowdBakerDataAsset;

USTRUCT(BlueprintType)
struct FCrowdBakerFrameData
{
	...
	* Frame to be played
	*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "CrowdBaker|Playback")
	float Frame = 0.0f;
...
USTRUCT(BlueprintType)
struct FCrowdBakerAutoPlayData
{
	GENERATED_USTRUCT_BODY()
...
UCLASS()
class CROWDBAKERRUNTIME_API UCrowdBakerInstancePlaybackLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "CrowdBaker|Playback")
	...
	*/
	...
	static bool BatchUpdateInstancesAutoPlayData(UInstancedStaticMeshComponent* InstancedMeshComponent,
		const TArray<FCrowdBakerAutoPlayData>& AutoPlayData, const TArray<FMatrix>& Transforms, bool bMarkRenderStateDirty=true);
	...
	*/
	...
	static bool BatchUpdateInstancesFrameData(UInstancedStaticMeshComponent* InstancedMeshComponent,
		const TArray<FCrowdBakerFrameData>& FrameData, const TArray<FMatrix>& Transforms, bool bMarkRenderStateDirty=true);
	...
	*/
	...
	static bool UpdateInstanceAutoPlayData(UInstancedStaticMeshComponent* InstancedMeshComponent, int32 InstanceIndex, 
		const FCrowdBakerAutoPlayData& AutoPlayData, bool bMarkRenderStateDirty=true);
	...
	*/
	...
	static bool UpdateInstanceFrameData(UInstancedStaticMeshComponent* InstancedMeshComponent, int32 InstanceIndex, 
		const FCrowdBakerFrameData& FrameData, bool bMarkRenderStateDirty=true);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "CrowdBaker|Playback")
	static bool GetAutoPlayDataFromDataAsset(const UCrowdBakerDataAsset* DataAsset, int32 AnimationIndex,
		FCrowdBakerAutoPlayData& AutoPlayData, float TimeOffset = 0.f, float PlayRate = 1.f);
	...
	UFUNCTION(BlueprintCallable, Category = "CrowdBaker|Playback")
	static bool GetFrameDataFromDataAsset(const UCrowdBakerDataAsset* DataAsset, int32 AnimationIndex, float Time,
		FCrowdBakerFrameData& AutoPlayData, float TimeOffset = 0.f, float PlayRate = 1.f);
...
template <class DataType>
bool UCrowdBakerInstancePlaybackLibrary::BatchUpdateInstancesData(UInstancedStaticMeshComponent* InstancedMeshComponent,
...
template <class DataType>
void UCrowdBakerInstancePlaybackLibrary::GetData(const DataType& Data, TArray<float>& CustomData)
{
...
template <class DataType>
void UCrowdBakerInstancePlaybackLibrary::GetCustomData(const TArray<DataType>& Data, TArray<float>& CustomData)
{
...
template <class DataType>
bool UCrowdBakerInstancePlaybackLibrary::UpdateInstanceCustomData(UInstancedStaticMeshComponent* InstancedMeshComponent, int32 InstanceIndex,
