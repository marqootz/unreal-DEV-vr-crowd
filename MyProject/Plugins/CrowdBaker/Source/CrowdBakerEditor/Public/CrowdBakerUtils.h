...
// LowPrecision
template<>
FORCEINLINE void CrowdBaker_Private::VectorToColor(const FVector3f& Vector, FColor& Color)
{
...
// LowPrecision
template<>
FORCEINLINE void CrowdBaker_Private::VectorToColor(const FVector4f& Vector, FColor& Color)
{
	const float ClampedX = FMath::Clamp(Vector.X, 0.f, 1.f);
	const float ClampedY = FMath::Clamp(Vector.Y, 0.f, 1.f);
	const float ClampedZ = FMath::Clamp(Vector.Z, 0.f, 1.f);
...
// HighPrecision
template<>
FORCEINLINE void CrowdBaker_Private::VectorToColor(const FVector3f& Vector, FVector4u16& Color)
{
...
// HighPrecision
template<>
FORCEINLINE void CrowdBaker_Private::VectorToColor(const FVector4f& Vector, FVector4u16& Color)
{
	Color.X = FMath::RoundToInt(FMath::Clamp(Vector.X, 0.f, 1.f) * TNumericLimits<uint16>::Max());
	Color.Y = FMath::RoundToInt(FMath::Clamp(Vector.Y, 0.f, 1.f) * TNumericLimits<uint16>::Max());
	Color.Z = FMath::RoundToInt(FMath::Clamp(Vector.Z, 0.f, 1.f) * TNumericLimits<uint16>::Max());
...
template<class V, class TextureSettings>
FORCEINLINE_DEBUGGABLE bool CrowdBaker_Private::WriteVectorsToTexture(const TArray<V>& Vectors,
	const int32 NumFrames, const int32 RowsPerFrame,
...
template<class TextureSettings>
FORCEINLINE_DEBUGGABLE bool CrowdBaker_Private::WriteSkinWeightsToTexture(const TArray<CrowdBaker_Private::VertexSkinWeightFour>& SkinWeights, const int32 NumBones,
...
template<class TextureSettings>
FORCEINLINE_DEBUGGABLE bool CrowdBaker_Private::WriteToTexture(
	UTexture2D* Texture,
