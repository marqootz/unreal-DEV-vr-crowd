...
#include "CrowdBakerBPLibrary.generated.h"
...
*/
UCLASS()
class UCrowdBakerBPLibrary : public UBlueprintFunctionLibrary
{
	...
	*/
	UFUNCTION(BlueprintCallable, meta = (Category = "CrowdBaker"))
	static CROWDBAKEREDITOR_API UPARAM(DisplayName="bSuccess") bool AnimationToTexture(UCrowdBakerDataAsset* DataAsset);

	/** 
	* Utility for converting SkeletalMesh into a StaticMesh
	*/
	UFUNCTION(BlueprintCallable, Category = "CrowdBaker")
	static CROWDBAKEREDITOR_API UStaticMesh* ConvertSkeletalMeshToStaticMesh(USkeletalMesh* SkeletalMesh, const FString PackageName, const int32 LODIndex = -1);

	/**
	* Utility for setting a StaticMesh LightMapIndex.
	*/
	UFUNCTION(BlueprintCallable, Category = "CrowdBaker")
	static CROWDBAKEREDITOR_API UPARAM(DisplayName = "bSuccess") bool SetLightMapIndex(UStaticMesh* StaticMesh, const int32 LODIndex, const int32 LightmapIndex=1, bool bGenerateLightmapUVs=true);

	/**
	* Updates a material's parameters to match those of an CrowdBaker DataAsset
	*/
	UFUNCTION(BlueprintCallable, meta = (Category = "CrowdBaker"))
	static CROWDBAKEREDITOR_API void UpdateMaterialInstanceFromDataAsset(const UCrowdBakerDataAsset* DataAsset, class UMaterialInstanceConstant* MaterialInstance,
...
private:
	...
	static bool CheckDataAsset(const UCrowdBakerDataAsset* DataAsset,
	...
	static int32 GetAnimationFrameRange(const FCrowdBakerAnimSequenceInfo& Animation, 
